...
contract HyperpaymentV1 is ResourceFlow {
    uint constant public SCORE_TOKEN = 1;
    uint constant public NATIVE_TOKEN = 2;
    ...
    struct Resource {
        // 0x01 is ETH, otherwise ERC20 token address.
        // The token is the resource type.
        address token;  
    }
    ...
    function hyperpay(uint specID, uint projectID, bytes calldata payload) external {
        // require(projects[specID][projectID], "Project is not registered");
    ...
    function processSpline(uint specID, uint projectID, uint splineID, uint counter) internal {
        ...
        transferToCategory(
            ...
            resourceAmount,
            specs[specID].resources[spline.flow.to].token
        );
        categoryContract.paycheck(specID, projectID, splineID, counter, specs[specID].resources[spline.flow.to].token, resourceAmount);
    ...
    function splitProducts(uint specID, Flow memory flow) internal {
        ...
        Resource storage fromResource = spec.resources[flow.from];
        Resource storage toResource = spec.resources[flow.to];
        if (fromResource.token == toResource.token) {
    ...
    function convertSameResource(Flow memory flow) internal {
        ...
        uint toResourceAmount = (flow.percentage * perPercentage);
        ...
        storeProduct(flow.to, toResourceAmount, 100, toResourceAmount / 100);
    }
    ...
    function getCategoryContract(uint specID, uint splineID) internal view returns (ICategoryContract) {
