 ...
 * @notice
 */
contract CascadeAccount2 is AccessControlUpgradeable {
    struct Account {
    ...
    function withdrawAllToken(string memory purl, address token) external onlyWithdrawer(purl) {
        ...
        require(cascadeAccounts[purl].withdrawer != address(0), "Withdrawer not set");
        IERC20(token).transfer(cascadeAccounts[purl].withdrawer, cascadeAccounts[purl].balances[token]);
